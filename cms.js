(() => {
  ...
  async function loadArticle(){
    ...
    target.innerHTML=`<header class="cms-detail-head"><span class="tag">${esc(a.category||'Culture')}</span><p class="cms-date">${esc(dateLabel(a.published_at||a.created_at))}</p><h1>${esc(pick(a,'title'))}</h1><p class="lead">${esc(pick(a,'summary'))}</p></header>${a.cover_image?`<img class="cms-detail-cover" src="${esc(a.cover_image)}" alt="${esc(pick(a,'title'))}">`:''}<div class="cms-detail-body">${textToHtml(pick(a,'content'))}</div>`;
  }
