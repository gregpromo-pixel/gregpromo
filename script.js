...
function setupInstagramLink(anchor, url, label){
  ...
  anchor.innerHTML = INSTAGRAM_ICON + `<span class="sr-only">${label}</span>`;
...
function normalizeContactInformation(){
  ...
  document.querySelectorAll('[data-contact-email]').forEach(link=>{
    ...
    if(!link.querySelector('strong')) link.textContent = email;
    else link.querySelector('strong').textContent = email;
  });

  document.querySelectorAll('[data-contact-phone]').forEach(link=>{
    link.href = `tel:${phoneHref}`;
    if(!link.querySelector('strong')) link.textContent = phoneDisplay;
    else link.querySelector('strong').textContent = phoneDisplay;
  });

  document.querySelectorAll('a[href^="mailto:"]').forEach(link=>{
    const current = link.getAttribute('href') || '';
    if(/gmail\.com|yahoo\.com|outlook\.com|hotmail\.com|example\.com|booking@gregpromo\.com/i.test(current)){
      link.href = `mailto:${email}`;
      if(!link.querySelector('strong')) link.textContent = email;
    }
  });

  const legacyEmailPattern = /[A-Z0-9._%+-]+@(gmail\.com|yahoo\.com|outlook\.com|hotmail\.com|example\.com|gregpromo\.com)/gi;
  const walker = document.createTreeWalker(document.body, NodeFilter.SHOW_TEXT, {
    acceptNode(node){
      const parent = node.parentElement;
      if(!parent || ['SCRIPT','STYLE','CODE','TEXTAREA','INPUT','SELECT'].includes(parent.tagName)) return NodeFilter.FILTER_REJECT;
      return legacyEmailPattern.test(node.nodeValue) ? NodeFilter.FILTER_ACCEPT : NodeFilter.FILTER_SKIP;
    }
  });
  const nodes=[];
  while(walker.nextNode()) nodes.push(walker.currentNode);
  nodes.forEach(node=>{ node.nodeValue = node.nodeValue.replace(legacyEmailPattern, email); });
}
