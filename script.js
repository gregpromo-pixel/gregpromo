...
function bindLangButtons(){
  document.querySelectorAll('[data-lang-btn]').forEach(btn=>{
    ...
    btn.addEventListener('click', ()=>{
      ...
      localStorage.setItem('gp-lang', btn.dataset.langBtn);
      applyI18n();
...
function setupLeagueButtons(){
  ...
  box.querySelectorAll('[data-league]').forEach(btn => btn.addEventListener('click', ()=>{
...
function updateActiveLeague(){
  document.querySelectorAll('[data-league]').forEach(btn => btn.classList.toggle('is-active', btn.dataset.league === selectedCompetition));
...
function renderFixtures(list){
  ...
  const html = rows.length ? rows.map(item=>{
    ...
    const date = item.fixture?.date ? new Date(item.fixture.date).toLocaleString([], { dateStyle:'medium', timeStyle:'short' }) : 'TBD';
